...
async function updateVoiceStatus(client, player, statusText) {
    if (!player || !player.voiceId) return;

    const status = statusText || `♪ ${client.user.username} Music`;
...
module.exports = (client) => {
    ...
    client.manager.on("playerStart", async (player, track) => {
        await updateVoiceStatus(client, player, `Playing: ${track.title}`);
    });
    ...
    client.manager.on("playerEnd", async (player, track, payload) => {
        ...
        setTimeout(() => updateVoiceStatus(client, player, null), 500);
    });
