...
class MainClient extends Client {
  ...
  async loadEvents() {
    fs.readdirSync("./src/events/").forEach((file) => {
      ...
      try {
        const eventPath = `${process.cwd()}/src/events/${file}`;
        const eventModule = require(eventPath);

        // Support CommonJS function export or default function export
        const handler =
        ...
        if (handler) {
          ...
          console.warn(`[ SKIPPED ] ${eventName}.js does not export a function`);
        }
      } catch (err) {
